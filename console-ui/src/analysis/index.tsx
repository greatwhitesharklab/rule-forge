...
interface FlowTrendChartProps {
    data: FlowTimeseriesData;
}

class FlowTrendChart extends Component<FlowTrendChartProps> {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    componentDidUpdate(prev: FlowTrendChartProps) {
    ...
    updateChart(props: FlowTrendChartProps) {
        const {data} = props;
...
interface RejectDistributionChartProps {
    data: RejectDistributionRow[];
}

class RejectDistributionChart extends Component<RejectDistributionChartProps> {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    private chart: any = null;
    private container: HTMLDivElement | null = null;
    ...
    componentDidUpdate(prev: RejectDistributionChartProps) {
        if (prev.data !== this.props.data) this.updateChart(this.props);
    }
    componentWillUnmount() {
        window.removeEventListener('resize', this.handleResize);
    ...
    handleResize = () => { if (this.chart) this.chart.resize(); };
    updateChart(props: RejectDistributionChartProps) {
        const {data} = props;
...
interface RuleFireChartProps {
    data: RuleCoverageData;
}

class RuleFireChart extends Component<RuleFireChartProps> {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    private chart: any = null;
    private container: HTMLDivElement | null = null;
    ...
    componentDidUpdate(prev: RuleFireChartProps) {
        if (prev.data !== this.props.data) this.updateChart(this.props);
    }
    componentWillUnmount() {
        window.removeEventListener('resize', this.handleResize);
    ...
    handleResize = () => { if (this.chart) this.chart.resize(); };
    updateChart(props: RuleFireChartProps) {
        const {data} = props;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type ThunkDispatch = (thunkOrAction: any) => any;

interface AnalysisDashboardProps extends AnalysisState {
    dispatch: ThunkDispatch;
}

class AnalysisDashboard extends Component<AnalysisDashboardProps> {
    componentDidMount() {
        this.refreshData();
    }
    componentDidUpdate(prevProps: AnalysisDashboardProps) {
    ...
    handleTimeRangeChange(range: string) {
        this.props.dispatch(action.setTimeRange(range as '1h' | '6h' | '24h' | '7d' | '30d'));
    }
    handlePackageChange(pkg: string) {
    ...
    renderCoverageTab(ruleCoverage: RuleCoverageData, ruleCoverageLoading: boolean) {
    ...
    renderAnomalyTab(anomalies: AnomalyRecord[], anomaliesLoading: boolean) {
        const severityColors: Record<string, string> = {HIGH: '#ff4d4f', MEDIUM: '#faad14', LOW: '#1890ff'};
        const directionLabels: Record<string, string> = {SPIKE: '↑ 升高', DROP: '↓ 降低'};
        return (
            <div>
...
function mapStateToProps(state: AnalysisState): AnalysisState {
    return state;
}
