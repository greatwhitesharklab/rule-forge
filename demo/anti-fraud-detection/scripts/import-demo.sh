...
# 创建项目
echo "创建演示项目..."
curl -sf -X POST "${API_BASE}/frame/saveProject" \
  -H "Content-Type: application/json" \
  -d "{
    \"name\": \"反欺诈交易检测\",
    \"description\": \"AI+规则混合的实时交易风控决策演示\",
    \"type\": \"demo\"
  }" > /dev/null && echo "✓ 项目创建完成" || echo "⚠ 项目可能已存在"
...
curl -sf -X POST "${API_BASE}/frame/publishPackage" \
  -H "Content-Type: application/json" \
  -d "{\"project\": \"反欺诈交易检测\", \"version\": \"1.0.0\"}" > /dev/null && echo "✓ 知识包发布成功" || echo "⚠ 知识包发布跳过"

echo ""
echo "=============================================="
echo "✓ Demo 导入完成！"
echo "请先运行 setup-model-service.sh 配置模型"
