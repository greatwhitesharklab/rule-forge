...
- Phase 1 实现:头配置是**数据驱动**的 —— 每个数据集在 `training/datasets.py`
  里声明自己的 `DatasetSpec`(特征清单、数值列、K 个头、分箱数),改数据集不改流水线。
  credit-g 的 4 头配置冻结为基线;GiveMeSomeCredit(150k)配 4 头:单特征头
  (90 天逾期次数)+ 三个二阶交叉头(利用率×30-59 逾期、负债比×收入段、年龄×信贷账户数),
  数值特征按训练集分位数分 10 箱后交叉。
