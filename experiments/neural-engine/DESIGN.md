...
判定对象:有编排器(0.6B + GRPO) vs 无编排器(写死策略 loop.py),CLAB 上相同轮数。

| 指标 | 无编排器 baseline | 有编排器要达到 | 判定 |
|---|---|---|---|
| 发现效率(有效特征/云端调用) | B_eff | **≥ B_eff × 1.3** | ❌ 否则失败 |
| 死路重复率 | B_rep | **≤ B_rep**(不高于 baseline,见注) | ❌ 否则失败 |
| 总有效特征数(固定轮数) | B_feat | **≥ B_feat** | ❌ 否则失败 |
| GRPO 收敛性 | — | reward 曲线不发散 | ❌ 否则失败 |

**注(B_rep 语义,2026-07 阶段 1.2 实测后修正)**:baseline 的 cloud
是 `ClabAutoCloud`(暴力枚举器,有内部去重),B_rep 天然 = 0。所以证伪标准
从"≤ baseline × 0.5"改为"≤ baseline"—— 编排器不能让重复率**变差**即可
(因为编排器用真实 LLM,LLM 可能重复提已死路方向,这是编排器要学的:
记住死路、出题时避开)。如果编排器 B_rep > 0 且无下降趋势 → 记忆机制无效。

**baseline 实测值(2026-07-28, seed=20260728, rounds=5)**:
- B_eff = 16.6(83 有效特征 / 5 云端调用)
- B_rep = 0.0(枚举器去重,天然 0)
- B_feat = 83

**硬规则**:四项任一不达标 → 阶段 1 失败,范式证伪,停。
GRPO 不收敛 → RL 路径证伪,评估换 SFT 兜底。
四项全达标 → 进阶段 2。
