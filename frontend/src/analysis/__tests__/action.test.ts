...
describe('analysis actions', () => {
    ...
    describe('sync action creators', () => {
        it('setTimeRange should create SET_TIME_RANGE action', () => {
            const act = setTimeRange('7d');
            expect(act).toEqual({type: SET_TIME_RANGE, range: '7d'});
        });

        it('setAnalysisPackage should create SET_ANALYSIS_PACKAGE action', () => {
            const act = setAnalysisPackage('loan-rules');
            expect(act).toEqual({type: SET_ANALYSIS_PACKAGE, pkg: 'loan-rules'});
        });

        it('setTab should create SET_TAB action', () => {
            const act = setTab('coverage');
            expect(act).toEqual({type: SET_TAB, tab: 'coverage'});
        });
    });
