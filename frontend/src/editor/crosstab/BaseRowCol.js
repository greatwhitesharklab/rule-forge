...
export default class BaseRowCol {
    /**
     * @param {CrossTable} table - The parent cross table instance
     */
    constructor(table) {
        ...
        for (const col of columns) {
            ...
            if (cell) {
                ...
                if (rowSpan > 1) {
                    ...
                    cell.row = nextRow;
                    if (nextRow.istop && nextRow.bundleData) {
                        cell.setBundleData(nextRow.bundleData);
                    }
        ...
        for (const row of rows) {
            ...
            if (cell) {
                ...
                if (colSpan > 1) {
                    ...
                    cell.col = nextCol;
                    if (row.isleft && nextCol.bundleData) {
                        cell.setBundleData(nextCol.bundleData);
                    }
                ...
                for (let ci = colIndex - 1; ci > -1; ci--) {
                    ...
                    const prevCell = this.table.getCell(row, prevCol);
                    if (prevCell) break;
                }
                if (prevCell) {
     ...
     *
     * @param {Row} row - The row for the cell
     * @param {Column} col - The column for the cell
     * @param {BaseRowCol} parentRowCol - The parent row or column that owns the parent cell
     * @param {boolean} isTop - Whether this is a top (column-header) condition cell
     * @returns {ConditionCell}
     */
    newConditionCell(row, col, parentRowCol, isTop) {
     ...
     * @returns {Cell}
     */
    newValueCell(row, col, parentRowCol) {
     ...
     *
     * @param {Object} bundleData - The bundle data object
     * @param {string} bundleData.type - "variable" or "parameter"
     * @param {string} bundleData.variableCategory - Category name
     * @param {string} [bundleData.variableName] - Variable name
     * @param {string} [bundleData.variableLabel] - Display label
     * @param {string} [bundleData.datatype] - Data type
     * @returns {string} XML attribute string
     */
    bundleDataToXml(bundleData) {
