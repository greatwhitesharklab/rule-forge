...
export default class CrossTable {
     ...
     *
     * @param {Object} data - The crosstab data from the server
     * @param {Array} [data.rows] - Row definitions
     * @param {Array} [data.columns] - Column definitions
     * @param {Map} [data.cellsMap] - Map of "rowNumber,columnNumber" -> cell data
     * @param {Object} [data.headerCell] - Header cell data
     */
    init(data) {
        ...
        let firstColData = {columnNumber: 1};
        if (columns.length > 0) {
        ...
        for (let ri = 0; ri < this.rows.length; ri++) {
            ...
            for (let ci = 0; ci < this.columns.length; ci++) {
                ...
                const cellKey = row.rowNumber + ',' + col.columnNumber;
     ...
     * @param {Cell} [parentCell] - Parent cell for hierarchical columns
     * @returns {TopColumn}
     */
    addNewTopColumn(parentCell) {
        return new TopColumn(this, parentCell);
    }

    /**
     * Add a new left (value) row.
     * @param {Cell} [parentCell] - Parent cell for hierarchical rows
     * @returns {LeftRow}
     */
    addNewLeftRow(parentCell) {
     ...
     * @param {Row} row
     * @param {Column} col
     * @returns {Cell|ConditionCell|undefined}
     */
    getCell(row, col) {
     ...
     */
    adjustHeaderCellColSpan(increment) {
        if (this.headerCell) {
     ...
     * @param {boolean} increment - True to increment, false to decrement
     */
    adjustHeaderCellRowSpan(increment) {
        if (this.headerCell) {
     ...
     * @returns {string|void} XML string, or void on error (shows alert)
     */
    toXml() {
        try {
