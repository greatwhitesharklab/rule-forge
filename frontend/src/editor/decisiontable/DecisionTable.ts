/**
 * DecisionTable — the main editor class for the decision-table rule type.
 *
 * Previously `RuleForge.DecisionTable` (IIFE).
 */
...
import { CellExecuteMethod } from './CellExecuteMethod.js';
...
// ruleforge namespace — still-JS files attach their constructors here
interface RuleforgeNamespace {
    Context: new (container: HTMLElement) => any;
    InputType: new (config: null, label: string) => any;
    ComparisonOperator: new (cb: () => void) => any;
    MethodAction: new () => any;
    Join: any;
    Connection: any;
    Condition: any;
    CellCondition: any;
    CellContent: any;
    CellExecuteMethod: any;
    RuleProperty: new (table: DecisionTable, name: string, value: any, editorType: number) => { getContainer: () => HTMLElement; toXml: () => string };
    ConfigVariableDialog: new (table: DecisionTable) => { open: () => void };
    ConfigConstantDialog: new (table: DecisionTable) => { open: () => void };
    ConfigActionDialog: new (table: DecisionTable) => { open: () => void };
    ConfigParameterDialog: new (table: DecisionTable) => { open: () => void };
}
...
// Declare MsgBox global (used in the clean menu handler)
...
export class DecisionTable {
    ...
    container: HTMLElement;
    remark: Remark;
    properties: Array<{ getContainer: () => HTMLElement; toXml: () => string }> = [];
    ...
    constructor(id: string) {
        ...
        const saveButton = `<div class="btn-group btn-group-sm navbar-btn" style="margin-top:0;margin-bottom: 0" role="group" aria-label="...">
        ...
        const testButton = '<div class="btn-group btn-group-sm navbar-btn" style="margin-left:5px;margin-top:0;margin-bottom: 0" role="group" aria-label="...">' +
            '<button id="testButton" type="button" class="btn btn-success navbar-btn"><i class="glyphicon glyphicon-flash"/> 快速测试</button>' +
            '</div>';
        const addCriteriaButton = `<button id="addCriteriaButton" type="button" class="btn btn-default btn-sm"><i class="glyphicon glyphicon-plus"/> 添加条件行</button>`;
        const deleteCriteriaButton = `<button id="deleteCriteriaButton" type="button" class="btn btn-default btn-sm"><i class="glyphicon glyphicon-minus"/> 删除条件行</button>`;
        ...
        const onClickProp = function (menuItem: any): void {
        ...
        self.addHook('afterRender', function (): void {
            self._dom.querySelectorAll('.htCore tr').forEach(function (tr: Element) {
                ...
                if (criteriaCol) {
                    (criteriaCol as HTMLElement).style.borderRightWidth = '4px';
                }
            });
        });
        self.initMenu();
    ...
    getMergeInfo(row: number, col: number): any {
        const plugin = this._handsontable.getPlugin('mergeCells');
    ...
    setMergeInfo(info: { row: number; col: number; rowspan: number; colspan: number }): void {
        const plugin = this._handsontable.getPlugin('mergeCells');
        plugin.merge(info.row, info.col, info.row + info.rowspan - 1, info.col + info.colspan - 1);
    }

    removeMergeInfo(row: number, col: number): void {
        const plugin = this._handsontable.getPlugin('mergeCells');
    ...
    insertCol(index: number): void {
        ...
        rows.forEach(function (tr: Element) {
    ...
    getCellContent(rowOrCellData: number | CellData, col?: number): any {
        let cellData: CellData;
        if (typeof rowOrCellData === 'object') {
            cellData = rowOrCellData;
        } else {
            cellData = this.getCellData(rowOrCellData, col!)!;
        }
        if (!cellData.cellContent) {
            const colData = this.getColData(cellData.col)!,
                type = colData.type,
                container = cellData.container;
            if (type === 'Criteria') {
                cellData.cellContent = new CellCondition('<div/>');
                if (cellData.joint) {
                    cellData.cellContent.initData(cellData.joint);
                }
            } else if (type === 'Assignment') {
                cellData.cellContent = new CellContent(container);
                if (cellData.value) {
                ...
                cellData.cellContent = new CellExecuteMethod(container);
                if (cellData.action) {
    ...
    initMenu(): void {
        ...
        const oldVariableLibrary = (window as unknown as Record<string, any[]>)._ruleforgeEditorVariableLibraries || [];
        oldVariableLibrary.forEach(function (lib: any): void {
            if (lib.type !== 'parameter') {
                variableLibrary.push(lib);
            }
        });
        const parameter = (window as unknown as Record<string, any[]>)._ruleforgeEditorParameterLibraries || [];
        ...
        const onInsert = function (type: string, width?: number, add?: number): void {
            ...
            self.renderCells(undefined, col);
            self.setDirty();
            self.invoke('render');
        };
        ...
        const menuItems: any[] = [{
            ...
            onClick: function (): void {
                ...
                const cellData = self.getCurrentCellData(), content = self.getCellContent(cellData);
            ...
            onClick: function (): void {
                const cellData = self.getCurrentCellData();
                MsgBox.confirm('确定要清空当前单元格吗？', function (): void {
        ...
        this.container.addEventListener('contextmenu', function (e: MouseEvent): void {
...
(window as any).RuleForge.DecisionTable = DecisionTable;
