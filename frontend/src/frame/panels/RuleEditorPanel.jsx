...
export default class RuleEditorPanel extends Component {
    constructor(props) {
        super(props);
        this.state = {
            projects: [],
    ...
    componentDidMount() {
        ...
        eventObj.eventEmitter.on(event.PROJECT_SELECT, projectName => {
        ...
        if (this.state.projects.length === 0) {
            this.props.store.dispatch(ACTIONS.loadData(true));
        }
    }
    ...
    _handleClickOutside(e) {
        if (this.state.dropdownOpen && !e.target.closest('.panel-project-selector')) {
