...
export default class AlertRulePanel extends Component {
    constructor(props) {
        super(props);
        this.state = {
            showForm: false,
            editingRule: null,
            form: {name: '', metricName: 'rule.execution.latency', condition: 'GT', threshold: 5000, durationMin: 1, webhookUrl: '', cooldownMin: 10}
        };
    }
    ...
    handleSave() {
        ...
        this.props.dispatch({type: 'SAVE_ALERT_RULE', rule, callback: () => this.setState({showForm: false})});
    }

    handleDelete(id) {
        if (window.confirm('确定删除此告警规则？')) {
            this.props.dispatch({type: 'DELETE_ALERT_RULE', id});
        }
    }
