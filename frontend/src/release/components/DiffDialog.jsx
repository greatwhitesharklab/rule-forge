...
const DiffDialog = {
    show(projectName, fromVersion, toVersion, filePath) {
        ...
        .then(data => {
            ...
            const content = diffs.filter(Boolean).map(d => d.patch || '').join('\n');

            if (content) {
                try {
                    const html = Diff2Html.getPrettyHtml(content, {
                        inputFormat: 'diff',
                    ...
                    window.bootbox.dialog({
                        ...
                        message: '<div style="max-height:70vh;overflow:auto;">' + html + '</div>',
                        size: 'large',
    ...
    _showPlainText(content, fromVersion, toVersion) {
