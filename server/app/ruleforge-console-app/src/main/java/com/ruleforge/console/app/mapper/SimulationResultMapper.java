...
public interface SimulationResultMapper extends BaseMapper<SimulationResultEntity> {
    ...
    @Select("SELECT * FROM rfa_simulation_result WHERE simulation_run_id = #{runId} " +
            "ORDER BY id DESC LIMIT #{limit} OFFSET #{offset}")
    List<SimulationResultEntity> selectByRunId(@Param("runId") Long runId,
                                                @Param("limit") int limit,
                                                @Param("offset") int offset);

    @Select("SELECT divergence_severity, COUNT(*) AS cnt FROM rfa_simulation_result " +
            "WHERE simulation_run_id = #{runId} GROUP BY divergence_severity")
    List<Map<String, Object>> countBySeverity(@Param("runId") Long runId);

    @Select("SELECT COUNT(*) FROM rfa_simulation_result WHERE simulation_run_id = #{runId} AND has_divergence = 1")
    int countDivergent(@Param("runId") Long runId);

    @Select("SELECT COUNT(*) FROM rfa_simulation_result WHERE simulation_run_id = #{runId}")
