...
--
-- 旧:BatchTest 单一 FLOW 模式,input 必须先 Excel 导入存到 rfa_batch_test_row
-- 新:同一个表支持 3 种组合
...
-- ── rfa_batch_test_session 加列 ─────────────────────────────────────
CALL v580_add_column_if_missing('rfa_batch_test_session', 'subject_type',
    'subject_type VARCHAR(32) NOT NULL DEFAULT ''FLOW'' COMMENT ''FLOW / DATASOURCE — 测什么''');
CALL v580_add_column_if_missing('rfa_batch_test_session', 'subject_id',
    'subject_id BIGINT NULL COMMENT ''flowId 或 datasourceId,跟 subject_type 一起定位被测对象''');
CALL v580_add_column_if_missing('rfa_batch_test_session', 'input_source_type',
    'input_source_type VARCHAR(32) NOT NULL DEFAULT ''FILE'' COMMENT ''FILE / DATASOURCE — input 从哪来''');
CALL v580_add_column_if_missing('rfa_batch_test_session', 'input_source_id',
    'input_source_id BIGINT NULL COMMENT ''datasourceId(FLOW+DATASOURCE 或 DATASOURCE+DATASOURCE 时填)''');
CALL v580_add_column_if_missing('rfa_batch_test_session', 'input_payload',
    'input_payload MEDIUMTEXT NULL COMMENT ''批量输入 JSON(FLOW+DATASOURCE 时存调三方拿到的 rows;FLOW+FILE 留空从 rfa_batch_test_row 读)''');

-- ── rfa_batch_test_row 加列 ────────────────────────────────────────
CALL v580_add_column_if_missing('rfa_batch_test_row', 'latency_ms',
    'latency_ms BIGINT NULL COMMENT ''单条执行耗时 ms''');
CALL v580_add_column_if_missing('rfa_batch_test_row', 'http_status',
    'http_status INT NULL COMMENT ''HTTP 状态码(仅 DATASOURCE 模式有)''');
CALL v580_add_column_if_missing('rfa_batch_test_row', 'error_code',
    'error_code VARCHAR(64) NULL COMMENT ''错误码(FLOW = RuleException label, DATASOURCE = HTTP/connector code)''');

-- ── 索引(用同样的预检查)─────────────────────────────────────────
SET @idx_exists = (SELECT COUNT(*) FROM information_schema.statistics
                   WHERE table_schema = DATABASE()
                     AND table_name = 'rfa_batch_test_session'
                     AND index_name = 'idx_batch_test_session_subject_type');
SET @sql = IF(@idx_exists = 0,
              'CREATE INDEX idx_batch_test_session_subject_type ON rfa_batch_test_session (subject_type, create_time)',
              'SELECT 1');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
--   input_payload 留 NULL(老 session 的 input 都在 rfa_batch_test_row 里)
