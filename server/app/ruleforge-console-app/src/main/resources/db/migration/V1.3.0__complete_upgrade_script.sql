...
-- 2. 修改rf_file_relation表，添加project_id字段
ALTER TABLE rf_file_relation
ADD COLUMN project_id bigint NULL COMMENT '项目ID' AFTER id;

-- 3. 修改rf_file_version表，添加缺失字段
...
-- 6. 创建知识包版本表
CREATE TABLE rf_package_version (
    id bigint AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    package_id varchar(64) NOT NULL COMMENT '知识包ID',
    name varchar(128) NULL COMMENT '知识包名称',
    resource_items text NULL COMMENT '资源项列表',
...
-- 7. 创建项目版本表
CREATE TABLE rf_project_version (
    id bigint AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    project_id bigint NOT NULL COMMENT '项目ID',
...
-- 8. 创建项目版本映射表
CREATE TABLE rf_project_version_mapping (
    id bigint AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
...
-- 9. 创建项目运行时配置表
CREATE TABLE rf_project_runtime_config (
    id bigint AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    project_id bigint NOT NULL COMMENT '项目ID',
    package_id varchar(64) NULL COMMENT '知识包ID',
...
-- 10. 创建项目运行时流程表
CREATE TABLE rf_project_runtime_flow (
    id bigint AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    project_id bigint NOT NULL COMMENT '项目ID',
    package_id varchar(64) NULL COMMENT '知识包ID',
    project_version varchar(32) NULL COMMENT '项目版本',
...
-- 11. 创建项目导入流程表
CREATE TABLE rf_project_import_flow (
    id bigint AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
...
-- 12. 初始化数据：将rf_file表中file_type=1的记录迁移到rf_project表（如果还没有的话）
INSERT INTO rf_project(id, name, is_lock, create_time, update_time)
SELECT id, name, 0, create_time, NOW()
FROM rf_file
WHERE file_type = 1
AND id NOT IN (SELECT id FROM rf_project WHERE id IS NOT NULL);

-- 13. 更新rf_file表的project_id字段
UPDATE rf_file f
SET project_id = f.id
WHERE f.file_type = 1;

-- 14. 更新rf_file_relation表的project_id字段
UPDATE rf_file_relation fr
SET project_id = (
    SELECT f.project_id
    FROM rf_file f
...
ALTER TABLE rf_file_version MODIFY COLUMN version_num varchar(128) NULL;
