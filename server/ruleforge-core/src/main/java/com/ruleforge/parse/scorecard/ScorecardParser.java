	...
	private void rebuildScorecard(ScorecardDefinition card){
		...
		for(CardCell cell:cells){
			...
			if(StringUtils.isNotBlank(varName)){
				...
				String actualCategory = cell.getVariableCategory();
				if (StringUtils.isBlank(actualCategory)) {
					actualCategory = colCategory;
				}
				
				try {
					Variable variable=rulesRebuilder.getVariableByName(resLibraries.getVariableCategories(), actualCategory, varName, null);
					cell.setDatatype(variable.getType());
					cell.setVariableLabel(variable.getLabel());
				} catch (Exception e) {
					// 增强错误处理：提供更详细的错误信息
					String errorMsg = String.format("在评分卡单元格(行:%d, 列:%d)中找不到变量 '%s'，类别: '%s'", 
						cell.getRow(), cell.getCol(), varName, actualCategory);
